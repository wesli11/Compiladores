...
def p_varDeclSen2(p):
	'''D_de : Sprima'''
	#p[0] = varDeclSen2(p[1],"varDeclSen2")
	print "D_de 2"
...
def p_Sprima1(p):
        '''Sprima : WHILE PARENTESIS_AB E PARENTESIS_CE LLAVE_ABIERTA S_ese LLAVE_CERRADA'''
...
def p_S1(p):
        '''S_ese : ID ASIGNACION E PUNTO_Y_COMA'''
        #p[0] = S1(Id(p[1]),asig(p[2]),p[3],"S1")
        print "S_ese 1"

def p_S2(p):
        '''S_ese : IF PARENTESIS_AB E PARENTESIS_CE'''
        #p[0] = S2((p[3]),"if condicional")
        print "S_ese 2"

def p_S3(p):
        '''S_ese : RETURN E PUNTO_Y_COMA'''
        #p[0] = S3((p[2]),"return")
        print "return <<"

def p_S4(p):
        '''S_ese : WRITE PARENTESIS_AB B PARENTESIS_CE PUNTO_Y_COMA'''
        #p[0] = S4(p[3],"write")
        print "S_ese 4"

def p_S5(p):
        '''S_ese : PROMPT PARENTESIS_AB ID PARENTESIS_CE PUNTO_Y_COMA'''
